...
function main(option) {
  ...
  const max_video_length_in_minutes = option["max_video_length_in_minutes"];
  var ina_skyppy_data;
      ...
      fetch(`${api_url}api/status/${youTubeId}`)
        ...
        .then((data) => {
          ...
          if (data.status_description === "too long") {
            tl.innerHTML = `<div class="label-h" style="width:100%; color:red; padding-top:20px">"the video is too long - the online version only supports videos up to ${max_video_length_in_minutes} minutes long"</div>`;
          }
